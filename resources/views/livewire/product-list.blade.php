                ...
                <div>
                    ...
                    <div class="h-40 overflow-y-auto border rounded p-2">
                        @foreach($brands as $brand)
                            <div class="mb-1">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" wire:model="selectedBrands" 
            ...
            <div wire:loading.remove>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <div>
                        ...
                        <button wire:click="setView('grid')" class="px-3 py-1 rounded border @if($selectedView === 'grid') bg-blue-600 text-white @endif">
                            grid view
                        </button>
                        <button wire:click="setView('table')" class="px-3 py-1 rounded border @if($selectedView === 'table') bg-blue-600 text-white @endif">
                            table view
                        </button>
                         | 
                        <span class="mr-2">Sort by:</span>
                        <button wire:click="sortBy('name')" class="px-3 py-1 rounded border @if($sortField === 'name') bg-blue-500 text-white @endif">
                            Name
                            @if($sortField === 'name')
                                @if($sortDirection === 'asc') ↑ @else ↓ @endif
                            @endif
                        </button>
                        <button wire:click="sortBy('price')" class="px-3 py-1 rounded border @if($sortField === 'price') bg-blue-500 text-white @endif">
                            Price
                            @if($sortField === 'price')
                                @if($sortDirection === 'asc') ↑ @else ↓ @endif
                            @endif
                        </button>
                        <button wire:click="sortBy('stock')" class="px-3 py-1 rounded border @if($sortField === 'stock') bg-blue-500 text-white @endif">
                            Stock
                            @if($sortField === 'stock')
                                @if($sortDirection === 'asc') ↑ @else ↓ @endif
                            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 if @if($selectedView !== 'grid') hidden @endif">
                        ...
                        <div class="bg-white rounded shadow overflow-hidden hover:shadow-lg transition-shadow">
                            <div class="p-4">
                                ...
                                <div class="flex justify-between items-center">
                                    <span class="rounded-full px-2 py-1 text-xs @if($product->is_active) bg-green-100 text-green-800 @else bg-red-100 text-red-800 @endif">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="py-2 px-4 border">
                                        <div class="flex items-center">
                                            <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-10 h-10 mr-2 rounded">
                                        </div>
                                    </td>
                                    <td class="py-2 px-4 border">{{ $product->name }}</td>
                                    <td class="py-2 px-4 border">{{ $product->category->name }}</td>
                                    <td class="py-2 px-4 border">{{ $product->brand->name }}</td>
                                    <td class="py-2 px-4 border font-bold">฿{{ number_format($product->price, 2) }}</td>
                                    <td class="py-2 px-4 border @if($product->stock < 10) text-red-500 font-bold @endif">
                                        {{ $product->stock }}
                                    </td>
